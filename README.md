...
- 车头与车尾向端部渐降，最前、最后的底缘以相切曲线上收，减小厚重的垂直立面。端部中心截面厚度约为车头 **5.9 mm**、车尾 **6.2 mm**；前脸采用单层宽进气口。轮拱与贴床区不随端部一起压扁。此项为外形调整，未进行风阻仿真。
- 两个前大灯分置在车头左右外角，灯带跨越机盖上表面、前脸与侧面，在圆角处连续转折。每侧灯面均实测覆盖这三个面；侧面灯尾位于输出坐标约 **Y=±28.1 mm**。边框投影宽 **0.54 mm**、沿包角三轴凹深 **0.39 mm**，灯罩面凹深 **0.11 mm**；边框和灯罩的高差可在单色打印中保留轮廓，全部与车体连为一体。
...
预览及 FreeCAD 中的深色玻璃、深色灯框和浅色灯罩是实际几何面的显示配色。单色 STL/3MF 不含透明玻璃或多色材料。预览采用实际 CAD 面的法线，保留真实曲面和特征边界，避免平整面被错误显示成凹陷。
...
`scripts/check_delivery.py` 检查保存的 CAD 实体、两侧大灯在上表面、前脸和侧面的连续覆盖、主曲面平顺性、四角收拢、前后端部厚度及渐薄截面、四轮上方壳体覆盖、后座舱过渡坡度、座舱弧面与宽度、双座参考包络、门缝、前后贴床面、尾翼、打印尺寸及 STL/3MF 一致性。生成脚本检查两根轴完整旋转包络与车身的间隙。
...
- `scripts/headlights.py`：环绕前角的前大灯、浅凹边框以及三面连续覆盖检查。
