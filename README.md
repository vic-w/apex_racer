...
预览及 FreeCAD 中的深色玻璃是凹面的显示配色。单色 STL/3MF 不含透明玻璃或多色材料。
...
| 检查区域（输出坐标） | 实际平面贴床面积 |
| --- | ---: |
| 车头 X=48 至 60.3 mm | 约 93.6 mm² |
| 车尾 X=-60.6 至 -48 mm，Z<22.8 mm | 约 97.5 mm² |
...
`scripts/check_delivery.py` 检查保存的 CAD 实体、座舱弧面与宽度、双座参考包络、门缝、前后贴床面、尾翼、打印尺寸及 STL/3MF 一致性。生成脚本检查两根轴完整旋转包络与车身的间隙。
...
- `apex_racer_preview.png`：透视、侧视和正视预览。
